...
async def select_and_send_async(account_id: str, profile_path: str, message: str, dry_run: bool = False, timeout: int = 120) -> Dict:
    try:
        Path(getattr(config, "SCREENSHOTS_DIR", "screenshots")).mkdir(parents=True, exist_ok=True)
    except Exception:
        pass
    ...
    try:
        ...
        if used_persistent:
            ...
            if not logged:
                try:
                    debug_dir = Path(getattr(config, "LOGS_DIR", "logs")) / account_id
                    debug_dir.mkdir(parents=True, exist_ok=True)
                    try:
        ...
        try:
            if page and os.environ.get('DEBUG_DUMP_SCREENSHOT') == '1':
                try:
                    ...
                    fname = Path(getattr(config, "SCREENSHOTS_DIR", "screenshots")) / f"{account_id}_{timestamp}.png"
                    htmlname = Path(getattr(config, "SCREENSHOTS_DIR", "screenshots")) / f"{account_id}_{timestamp}.html"
                    try:
