...
async def add_account_task_async(session_id: str, profile_name: str):
    ...
    try:
        async with async_playwright() as pw:
            ...
            try:
                await page.goto("https://web.whatsapp.com", wait_until="networkidle", timeout=LOGIN_TIMEOUT * 1000)
            except Exception as e:
                debug_dir = Path("logs") / "add" / session_id
...
# (保持原实现)
# ... (fetch_visible_contacts_async 内容保持不变)
        ...
        async with async_playwright() as pw:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        debug_dir = Path("logs") / account_id
                        debug_dir.mkdir(parents=True, exist_ok=True)
                        await page.screenshot(path=str(debug_dir / f"goto_failed_{int(time.time())}.png"), full_page=True)
                        html = await page.content()
                        with open(debug_dir / f"goto_failed_{int(time.time())}.html", "w", encoding="utf-8") as fh:
                            fh.write(html)
                        logger.warning("select_and_send_async page.goto failed for account=%s, saved debug to %s", account_id, debug_dir)
                    except Exception as se:
                        logger.exception("Failed saving debug artifacts for account=%s: %s", account_id, se)
                    logger.exception("select_and_send_async page.goto exception for account=%s: %s", account_id, e)
                ...
                try:
                    ...
                    try:
                        ...
                        with open(debug_dir / f"no_chats_{int(time.time())}.html", "w", encoding="utf-8") as fh:
                            fh.write(html)
                        logger.warning("select_and_send_async no chats saved debug to %s", debug_dir)
                    except Exception:
                        pass
